...
/// Build a full node.
///
/// The actual "flavor", aka if it will use `Mainnet`, `Alphanet` or `Chaosnet` is determined based
/// on [`IdentifyVariant`] using the chain spec.
pub fn build_full(config: Configuration) -> Result<NewFullBase<Client>, ServiceError> {
	#[cfg(feature = "chaosnet-native")]
	if config.chain_spec.is_chaosnet() {
		return new_full_node::<chaosnet_runtime::RuntimeApi, ChaosnetExecutorDispatch>(
			config,
			|_, _| (),
		)
	...
	if config.chain_spec.is_alphanet() {
		return new_full_node::<alphanet_runtime::RuntimeApi, AlphanetExecutorDispatch>(
			config,
			|_, _| (),
		)
		.map(|NewFullBase { task_manager, .. }| task_manager)
	...
	#[cfg(feature = "mainnet-native")]
	{
		return new_full_node::<mainnet_runtime::RuntimeApi, MainnetExecutorDispatch>(
			config,
			|_, _| (),
		)
		.map(|NewFullBase { task_manager, .. }| task_manager)
...
/// Creates a full service from the configuration.
#[cfg(feature = "full-node")]
pub fn new_full_node<RuntimeApi, Executor>(
	mut config: Configuration,
	with_startup_data: impl FnOnce(
		&sc_consensus_babe::BabeBlockImport<
			Block,
