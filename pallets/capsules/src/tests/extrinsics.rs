...
fn set_ipfs_reference_happy() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // Happy path
            let ok =
                TernoaCapsules::set_ipfs_reference(alice.clone(), nft_id, new_reference.clone());
            assert_ok!(ok);
            assert_eq!(
                TernoaCapsules::capsules(nft_id).unwrap().ipfs_reference,
                new_reference
            );
        })
}
